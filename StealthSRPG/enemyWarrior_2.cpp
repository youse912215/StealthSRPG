...
EnemyWarrior_2::EnemyWarrior_2(int x, int y, int graph, int moving_quantity, int attack, int range, int act_time,
...
void EnemyWarrior_2::Update(vector<vector<int>>& map) {
	get_latency();
	wait_motion();
	activate_reset();
	score_decision();
...
void EnemyWarrior_2::Draw() {
	...
	DrawFormatString(710, 490, GetColor(200, 255, 125), "ene_cost:L%d, R%d, U%d, D%d",
	                 enemy_cost[LEFT], enemy_cost[RIGHT], enemy_cost[UP], enemy_cost[DOWN], false);
...
void EnemyWarrior_2::drawing_effect2() {
	if (attack_activity) {
...
void EnemyWarrior_2::get_each_node() {
...
void EnemyWarrior_2::get_survival_activity(const bool& p_s_activity, const bool& sw1_s_activity,
...
void EnemyWarrior_2::get_two_point_distance(const int& p_x, const int& p_y, const int& sw1_x, const int& sw1_y,
                                            const int& sw2_x,
...
void EnemyWarrior_2::get_minimum_husteric() {
...
void EnemyWarrior_2::get_node_husteric() {
...
void EnemyWarrior_2::get_obstacle_cost(vector<vector<int>>& map) {
	/* 左側のコスト */
...
void EnemyWarrior_2::get_relative_position_cost() {
	/* x方向の相対位置コスト */
...
void EnemyWarrior_2::get_node_cost() {
	cost[LEFT] = abs(
...
void EnemyWarrior_2::get_node_score() {
...
void EnemyWarrior_2::moving_decision() {
...
void EnemyWarrior_2::get_enemy_cost_1(const int& ex1, const int& ey1, const int& ex2, const int& ey2, const int& ex3,
...
void EnemyWarrior_2::get_enemy_cost_2(const int& ex1, const int& ey1, const int& ex2, const int& ey2, const int& ex3,
                                      const int& ey3, const int& ex4, const int& ey4, const int& ex5, const int& ey5,
                                      const int& ex6, const int& ey6, const int& ex7, const int& ey7, const int& ex8,
...
void EnemyWarrior_2::Dead(vector<vector<int>>& map) {
...
void EnemyWarrior_2::score_decision() {
	if (minimum_score == 1) {
		this->activity = true;
...
void EnemyWarrior_2::moving_end() {
	if (this->range == moving_distance) {
		this->activity = true;
	}
}

void EnemyWarrior_2::activate_reset() {
	if (Map::scene % 2 == 0) {
		this->activity = false;
		attack_activity = false;
...
void EnemyWarrior_2::duplicate_process(const int& dir_num) {
	enemy_cost[dir_num] = 2;
}

void EnemyWarrior_2::get_attack_direction(const int& player_num) {
...
void EnemyWarrior_2::get_slash_motion(const int& a_activity, int* motion) {
