...
ShadowWarrior_3::ShadowWarrior_3(int x, int y, int graph, int moving_distance, int hp, int range, bool activity,
                                 bool isAlive,
                                 Input& input):
...
void ShadowWarrior_3::Update(const int& p_x, const int& p_y, const int& sw1_x, const int& sw1_y,
	...
	Move();
	Pickup();
	duplicate_decision(p_x, p_y, sw1_x, sw1_y, sw2_x, sw2_y, ew1_x, ew1_y, eb1_x, eb1_y);
}
...
void ShadowWarrior_3::Draw() {
	...
	DrawFormatString(300, 45, GetColor(0, 0, 0), "兵3(%d, %d)", x / block_size, y / block_size, false);
